...
int main() {
    ...
    while (t--) {
        ...
        unordered_set<int> need, cur;
        int segments = 0;

        for (int x : a) {
            cur.insert(x);
            if (need.count(x)) need.erase(x);

            if (need.empty()) {       // সব 'প্রয়োজন' পূরণ, তাই এখানে কাটা যাবে ✂️
                ++segments;
                need.swap(cur);       // পরের সেগমেন্টের জন্য 'need' সেট করুন
                cur.clear();
            }
        }
        cout << segments << '\n';
