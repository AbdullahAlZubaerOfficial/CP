...
bool canPassAllDoors(vector<int>& doors, int x, int pressTime) {
    int n = doors.size();
    for (int i = 0; i < n; ++i) {
        if (doors[i] == 0) continue;
        // Door is closed
        if (i >= pressTime && i < pressTime + x) {
            continue; // Effect of button is active
        }
        return false; // Closed and button has no effect
    }
    return true;
}
...
int main() {
    ...
    while (t--) {
        ...
        for (int pressTime = 0; pressTime <= n; ++pressTime) {
            if (canPassAllDoors(doors, x, pressTime)) {
                possible = true;
                break;
            }
        }
